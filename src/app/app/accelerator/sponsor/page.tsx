...
async function createSponsorship(formData: FormData) {
  ...
  const acceleratorId = profile.entity_id;
  ...
  const { data: sponsorship } = await service
    ...
    .insert({
      sponsor_type:              "accelerator",
      sponsor_accelerator_id:    acceleratorId,
  ...
  try {
    ...
    await sendSponsorshipOffer(
      ...
      startup_name_input,
      accelerator?.org_name ?? "An accelerator",
      notes,
      portalUrl
    );
  } catch (e) {
    console.error("[createSponsorship/accelerator] email error", e);
  }

  revalidatePath("/app/accelerator/sponsor");
  revalidatePath("/app/accelerator/dashboard");
  redirect("/app/accelerator/dashboard");
}
...
export default async function AcceleratorSponsorPage() {
  ...
  const [{ data: accelerator }, { data: existingSponsorships }] = await Promise.all([
    service.from("accelerators").select("org_name, email").eq("id", acceleratorId).single(),
    service
      .from("accreditation_sponsorships")
      .select("id, startup_name_input, startup_email_input, status, created_at, notes")
      .eq("sponsor_accelerator_id", acceleratorId)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  type="email"
                  required
                  defaultValue={accelerator?.email ?? ""}
                  className="cs-input"
                />
              </div>
            </div>
